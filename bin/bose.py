...
class BluetoothctlProtocol(asyncio.SubprocessProtocol):
    ...
    async def disconnect(self, mac):
        await self.send_and_wait(
            ...
            fail_expression='Failed')

    async def connect(self, mac):
        await self.send_and_wait('connect %s' % (mac), 'Connection successful')

    async def pair(self, mac):
        await self.send_and_wait('pair %s' % (mac), 'Pair successful')
...
async def execute_command(cmd):
    p = await asyncio.create_subprocess_shell(
        cmd, stdout=sb.PIPE, stderr=sb.PIPE)
...
async def main():
    ...
    # Bose
    mac = '2C:41:A1:4B:E4:A5'
    device_id = 'bluez_card.{}'.format(mac.replace(':', '_'))
